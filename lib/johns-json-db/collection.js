...
class Collection {
  ...
  async init() {
    ...
    for (let file in files) {
      ...
      const withoutExtension = file.substring(0, file.lastIndexOf('.'));
  ...
  getAll(options = {}) {
    return (options.format !== 'object') ? Object.keys(this.documents).map(handle => {
      return this.documents[handle].get();
    }, this) : Object.keys(this.documents).reduce((acc, handle) => ({
      ...acc,
      [handle]: this.documents[handle].get()
    }), {}) ;
  }
  async deleteKeys(docName, keys) {
    await this.documents[docName].deleteKeys(keys);
  }
}
